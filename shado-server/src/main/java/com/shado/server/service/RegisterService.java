 ...
 * 
 */
package com.shado.server.service;
...
import com.shado.server.communication.VCarsMailer;
import com.shado.server.communication.VCarsSmsGateway;
import com.shado.server.constants.SMSTemplates;
...
public class RegisterService {
	...
	@Autowired
	private VCarsSmsGateway vCarsSmsGateway;
	
	@Autowired
	private VCarsMailer vCarsMailer;
	...
	public Register sendOTP(Register register){
		...
		try {
			...
			vCarsSmsGateway.sendSMS(register.getMobileNumber()+"", message);  
		...
		try {
			...
			vCarsMailer.sendEmail(email, subject, ccMail, bccMail, message);
	...
	public Register sendAcceptanceStatus(Register register){
		  
		logger.debug("In::sendAcceptanceStatus:"+register); 

		try {
			String message = "";  
			message = SMSTemplates.ACCEPTANCE_TEMPLATE .replaceAll("##STATUS##", register.getStatus()); 
			vCarsSmsGateway.sendSMS(register.getMobileNumber()+"", message);  
			logger.debug("Out::triggerAlertSMS");
		...
		try {
			...
			subject = "Your Profile Status Changed";
			ccMail = null;
			bccMail = null; 
						 
			message = this.getTemplate("acceptance-status", register);
			...
			e.printStackTrace();
			logger.error("In::sendAcceptanceStatus:"+e.getMessage());
		}  
		
		logger.debug("In::sendAcceptanceStatus");
		return register;
